  ...
  for (x in 1:length(datalist)) {
    ...
    tryCatch({
      ...
      stop(paste("Please close the file", datalist[[x]]))
          ...
          # Change to MRI code if the file exist.
          
          if (file.exists(file.path(ppdata, paste0(proj, "_id.xlsx")))){
          setwd(ppdata)
          id_change = read_excel(paste0(proj, "_id.xlsx"))
          setwd(prawdata)
          # First, create a lookup from the id_change data
          id_lookup <- setNames(id_change$new_id, id_change$old_id)
          
          # Replace participant_id values with new_id if they exist in id_lookup
          Data$participant_id <- ifelse(Data$participant_id %in% names(id_lookup),
                                        id_lookup[Data$participant_id], 
                                        Data$participant_id)
    ...
    # Generating participants.tsv
            ...
            for (c in 1:nrow(Data)) {
                  ...
                  # Select relevant data columns for demographics.tsv
                      pdata= select(Data,"participant_id", matches(paste0("^(", paste(na.omit(part$Itemname), collapse = "|"), ")$")))
                      pdata= pdata[c,]
                  ...
                  # Extract participant_id
                      ...
                      # Check if participant_id exists in part_empty
                      for (id in participant_id) {
                        # If participant_id exists, update the values
                        if (id %in% rownames(part_empty)) {
                          part_empty[id, names(pdata)] = pdata[pdata$participant_id == id, ]
                        } else {
                          # If participant_id does not exist, add a new row with pdata values
                          part_empty[id, names(pdata)] = pdata[pdata$participant_id == id, , drop = FALSE]
                        }
                      }
                  }
